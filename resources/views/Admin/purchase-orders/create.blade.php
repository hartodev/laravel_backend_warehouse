...
<div class="admin-page-head"><h2>Buat Purchase Order</h2></div>
    ...
    <div class="admin-form-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:20px;">
        <div>
            ...
            <select name="supplier_id" required class="admin-select">
                ...
                <option value="{{ $supplier->id }}" @selected(old('supplier_id') == $supplier->id)>{{ $supplier->name }} ({{ $supplier->code }})</option>
                @endforeach
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" name="payment_term" value="{{ old('payment_term') }}" class="admin-input" placeholder="cth. Net 30">
        </div>
        <div>
    ...
    <tr class="item-row">
        <td>
            <select name="items[__i__][product_id]" class="admin-select product-select" required>
                ...
                <option value="{{ $product->id }}" data-price="{{ $product->purchase_price }}">{{ $product->name }} ({{ $product->sku }}) — {{ $product->unit }}</option>
                @endforeach
            </select>
        </td>
        <td><input type="number" min="1" name="items[__i__][quantity_ordered]" class="admin-input qty-input" required></td>
        <td><input type="number" step="0.01" min="0" name="items[__i__][unit_price]" class="admin-input price-input" required></td>
        <td><input type="number" step="0.01" min="0" max="100" name="items[__i__][discount_percent]" class="admin-input discount-input" value="0"></td>
...
function addItemRow() {
    ...
    row.querySelectorAll('.qty-input, .price-input, .discount-input').forEach(el => el.addEventListener('input', recalc));
...
function recalc() {
    ...
    document.querySelectorAll('.item-row').forEach(row => {
        ...
        const rowSub = qty * price * (1 - disc / 100);
