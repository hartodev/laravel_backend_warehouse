...
<form method="GET" class="admin-filter-bar">
    <select name="warehouse_id" onchange="this.form.submit()" class="admin-select" style="max-width:200px;">
        ...
        <option value="{{ $wh->id }}" @selected((string) request('warehouse_id')===(string) $wh->id)>{{ $wh->name }}
        </option>
        @endforeach
    </select>
    ...
    <table class="admin-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span
                        class="admin-badge {{ str_contains($m->type, 'in') ? 'admin-badge-success' : (str_contains($m->type, 'out') ? 'admin-badge-danger' : 'admin-badge-info') }}">
            ...
            @empty
            <tr>
                <td colspan="9" class="cell-empty">Belum ada pergerakan stok.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
