...
@section('content')
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Selamat datang, {{ auth()->user()->name }}. {{ now()->isoFormat('dddd, D MMMM Y') }}
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('stocks.low-stock') }}" class="text-xs text-primary-700 hover:underline">Lihat semua
                    →</a>
            </div>
        ...
        <div class="card">
            ...
            <div class="divide-y divide-gray-100 max-h-72 overflow-y-auto">
                @forelse($pendingBudgets as $br)
                    <div class="px-4 py-3 flex items-center justify-between gap-3 hover:bg-gray-50">
                        <div class="min-w-0">
                            <a href="{{ route('budget-requests.show', $br) }}"
                ...
                @forelse($activeTransfers->merge($pendingOpnames) as $item)
                    <div class="px-4 py-3 flex items-center justify-between gap-3 hover:bg-gray-50">
                        @if ($item instanceof \App\Models\StockTransfer)
                            <div class="min-w-0">
                                <a href="{{ route('stock-transfers.show', $item) }}"
                        ...
                        @else
                            <div class="min-w-0">
                                <a href="{{ route('stock-opnames.show', $item) }}"
