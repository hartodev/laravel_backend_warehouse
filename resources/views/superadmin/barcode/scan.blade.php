...
@section('breadcrumb')
<a href="{{ route('barcodes.index') }}" class="hover:text-primary-700">Barcode</a>
...
<div class="max-w-xl">
    ...
    <div class="card mb-5 border-green-300 bg-green-50">
        ...
        <div class="card-body space-y-2 text-sm">
            <div class="flex justify-between"><span class="text-gray-500">Nama</span><span class="font-bold">{{ $product->name }}</span></div>
            <div class="flex justify-between"><span class="text-gray-500">SKU</span><span class="font-mono">{{ $product->sku }}</span></div>
            <div class="flex justify-between"><span class="text-gray-500">Barcode</span><span class="font-mono">{{ $product->barcode }}</span></div>
            ...
            <div class="flex justify-between border-t pt-2 mt-2"><span class="text-gray-500">Stok di Gudang</span><span class="font-bold text-lg text-primary-700">{{ number_format($result['stockInfo']['quantity']) }}</span></div>
            @endif
        </div>
        <div class="card-body border-t">
            <a href="{{ route('products.show', $product) }}" class="btn-primary btn btn-sm">Lihat Produk →</a>
        </div>
    </div>
    @endif

    <div class="card">
        <div class="card-header"><h2 class="font-semibold text-gray-900">Scan Barcode Produk</h2></div>
        <form method="POST" action="{{ route('barcodes.do-scan') }}">
            ...
            <div class="card-body space-y-4">
                ...
                <div>
                    ...
                    <select name="scan_type" required class="form-select">
                        <option value="check"    {{ old('scan_type','check')==='check'    ? 'selected' : '' }}>Check / Pengecekan</option>
                        <option value="stock_in" {{ old('scan_type')==='stock_in' ? 'selected' : '' }}>Stok Masuk</option>
                        <option value="stock_out"{{ old('scan_type')==='stock_out'? 'selected' : '' }}>Stok Keluar</option>
                        <option value="transfer" {{ old('scan_type')==='transfer' ? 'selected' : '' }}>Transfer</option>
                        <option value="purchase" {{ old('scan_type')==='purchase' ? 'selected' : '' }}>Pembelian</option>
                    </select>
                </div>
                <div>
            ...
            <div class="card-body border-t flex justify-end gap-3">
                <a href="{{ route('barcodes.index') }}" class="btn-secondary btn">Log</a>
