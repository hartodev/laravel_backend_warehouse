...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <a href="{{ route('superadmin.budget-requests.create') }}" class="btn-primary btn">
...
<div class="card mb-5">
    <form method="GET" class="card-body flex flex-wrap gap-3 items-end">
        ...
        <a href="{{ route('superadmin.budget-requests.index') }}" class="btn-secondary btn">Reset</a>
    </form>
</div>
            ...
            <tr>
                ...
                <td class="text-right">
                    <div class="flex items-center justify-end gap-1">
                        <a href="{{ route('superadmin.budget-requests.show', $br) }}" class="btn btn-secondary btn-sm">Detail</a>
                        @if($br->status === 'draft')
                            <form method="POST" action="{{ route('superadmin.budget-requests.submit', $br) }}" class="inline">@csrf<button class="btn btn-primary btn-sm">Submit</button></form>
                        @elseif($br->status === 'pending')
                            <form method="POST" action="{{ route('superadmin.budget-requests.approve', $br) }}" class="inline">@csrf<button class="btn btn-success btn-sm">Setujui</button></form>
                    ...
                    <x-confirm-modal :id="'reject-br-'.$br->id" title="Tolak Pengajuan?" :message="'Pengajuan '.$br->nomor_form.' akan ditolak.'"
                        :action="route('superadmin.budget-requests.reject', $br)" method="POST" confirm-label="Tolak" confirm-class="btn-danger">
                        <div class="mt-3">
