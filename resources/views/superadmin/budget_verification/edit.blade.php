...
@section('breadcrumb')
    <a href="{{ route('superadmin.budget-verifications.index') }}" class="text-gray-500 hover:text-gray-700">Verifikasi Anggaran</a>
    <span class="text-gray-400 mx-1">/</span>
    <a href="{{ route('superadmin.budget-verifications.show', $budgetVerification) }}" class="text-gray-500 hover:text-gray-700">Detail</a>
    <span class="text-gray-400 mx-1">/</span>
        ...
        <div class="card-body">
            <div class="rounded-md bg-blue-50 border border-blue-200 p-4 text-sm text-blue-800">
                ...
                <div class="flex justify-between">
                    ...
                    <strong>Rp {{ number_format($budgetVerification->budgetRequest->total_estimasi, 0, ',', '.') }}</strong>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-5">
        ...
        <div class="card-body space-y-3">
            ...
            <div>
                ...
                <textarea name="doc_lainnya" rows="2" class="form-textarea">{{ old('doc_lainnya', $budgetVerification->doc_lainnya) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-5">
        ...
        <div class="card-body grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2">
                ...
                <textarea name="cek_anggaran" rows="3" class="form-textarea">{{ old('cek_anggaran', $budgetVerification->cek_anggaran) }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label class="form-label">Analisa Cashflow</label>
                <textarea name="analisa_cashflow" rows="3" class="form-textarea">{{ old('analisa_cashflow', $budgetVerification->analisa_cashflow) }}</textarea>
            </div>
            <div>
                <label class="form-label">Rekomendasi <span class="text-red-500">*</span></label>
                <select name="rekomendasi" class="form-select" required>
                    <option value="setuju" {{ old('rekomendasi', $budgetVerification->rekomendasi) === 'setuju' ? 'selected' : '' }}>Setuju</option>
                    <option value="tunda"  {{ old('rekomendasi', $budgetVerification->rekomendasi) === 'tunda'  ? 'selected' : '' }}>Tunda</option>
                    <option value="tolak"  {{ old('rekomendasi', $budgetVerification->rekomendasi) === 'tolak'  ? 'selected' : '' }}>Tolak</option>
                </select>
            </div>
            <div>
                <label class="form-label">Nominal Rekomendasi</label>
                <input type="number" step="0.01" min="0" name="nominal_rekomendasi"
                    value="{{ old('nominal_rekomendasi', $budgetVerification->nominal_rekomendasi) }}" class="form-input">
            </div>
            <div class="sm:col-span-2">
                <label class="form-label">Catatan Finance</label>
                <textarea name="catatan_finance" rows="3" class="form-textarea">{{ old('catatan_finance', $budgetVerification->catatan_finance) }}</textarea>
            </div>
        </div>
    </div>

    <div class="flex justify-end gap-3">
        <a href="{{ route('superadmin.budget-verifications.show', $budgetVerification) }}" class="btn btn-secondary">Batal</a>
