                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required
                            class="form-input @error('name') border-red-400 @enderror" placeholder="cth. Elektronik">
                        @error('name')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="form-label">Slug</label>
                        <input type="text" name="slug" value="{{ old('slug') }}" class="form-input font-mono"
                            placeholder="otomatis jika kosong">
                    </div>
                    <div>
                        <label class="form-label">Icon (emoji/class)</label>
                        <input type="text" name="icon" value="{{ old('icon') }}" class="form-input"
                            placeholder="cth. 📦 atau fa-box">
                    </div>
                </div>
                <div>
                    ...
                    <select name="parent_id" class="form-select">
                        ...
                        <option value="{{ $p->id }}" {{ old('parent_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}
                        </option>
                        @endforeach
                    </select>
                </div>
