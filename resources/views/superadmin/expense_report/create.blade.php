...
<div class="card mb-5">
    <div class="card-body">
        <form method="GET" action="{{ route('superadmin.budget_revisions.index') }}"
            class="flex flex-wrap gap-3 items-end">
            <div>
    ...
    <div class="card mb-5">
        ...
        <div class="card-body grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="alasan_revisi" rows="4" class="form-textarea"
                    required>{{ old('alasan_revisi', $budgetRevision->alasan_revisi) }}</textarea>
            </div>
        </div>
    </div>
...
<div class="max-w-2xl">
    <div class="card">
        <div class="card-header">
            ...
            <span
                class="badge {{ str_contains($activityLog->action,'create') ? 'badge-success' : (str_contains($activityLog->action,'delete') ? 'badge-danger' : 'badge-info') }}">
                {{ $activityLog->action }}
            </span>
        </div>
        <div class="card-body space-y-4 text-sm">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p>{{ $activityLog->created_at->isoFormat('D MMMM Y, HH:mm:ss') }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Model</p>
                    <p class="font-mono">{{ $activityLog->model_type ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Model ID</p>
                    <p class="font-mono">{{ $activityLog->model_id ?? '—' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-xs truncate">{{ $activityLog->user_agent ?? '—' }}</p>
                </div>
            </div>
            ...
            <div>
                ...
                <pre
                    class="bg-red-50 border border-red-100 rounded-lg p-3 text-xs overflow-x-auto text-red-800">{{ json_encode(json_decode($activityLog->old_values), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
            </div>
            @endif
            ...
            <div>
                ...
                <pre
                    class="bg-green-50 border border-green-100 rounded-lg p-3 text-xs overflow-x-auto text-green-800">{{ json_encode(json_decode($activityLog->new_values), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
            </div>
            @endif
