...
<div class="max-w-2xl">
    <div class="card">
        <div class="card-header">
            <h2 class="font-semibold text-gray-900">Edit Laporan Pertanggungjawaban</h2>
        </div>
        ...
        <form method="POST" action="{{ route('expense-reports.update', $expenseReport) }}">
            ...
            <div class="card-body space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Pengajuan terkait tidak dapat diubah setelah laporan dibuat.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nomor_invoice"
                            value="{{ old('nomor_invoice', $expenseReport->nomor_invoice) }}" class="form-input">
                    </div>
                    <div>
                        <label class="form-label">Nama Vendor</label>
                        <input type="text" name="nama_vendor"
                            value="{{ old('nama_vendor', $expenseReport->nama_vendor) }}" class="form-input">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="tanggal_transaksi"
                            value="{{ old('tanggal_transaksi', \Carbon\Carbon::parse($expenseReport->tanggal_transaksi)->format('Y-m-d')) }}"
                            class="form-input">
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="nominal_realisasi"
                            value="{{ old('nominal_realisasi', $expenseReport->nominal_realisasi) }}"
                            class="form-input">
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="lamp_invoice" value="1"
                                {{ old('lamp_invoice', $expenseReport->lamp_invoice) ? 'checked' : '' }}>
                            Invoice
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="lamp_bukti_transfer" value="1"
                                {{ old('lamp_bukti_transfer', $expenseReport->lamp_bukti_transfer) ? 'checked' : '' }}>
                            Bukti Transfer
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="lamp_kartu_garansi" value="1"
                                {{ old('lamp_kartu_garansi', $expenseReport->lamp_kartu_garansi) ? 'checked' : '' }}>
                            Kartu Garansi
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" name="lamp_serah_terima" value="1"
                                {{ old('lamp_serah_terima', $expenseReport->lamp_serah_terima) ? 'checked' : '' }}>
                            Serah Terima
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="lamp_lainnya"
                        value="{{ old('lamp_lainnya', $expenseReport->lamp_lainnya) }}" class="form-input">
                </div>
                ...
                <div>
                    ...
                    <ul class="text-sm text-primary-700 space-y-1">
                        ...
                        <li><a href="{{ asset('storage/' . $path) }}" target="_blank"
                                class="hover:underline">{{ basename($path) }}</a></li>
                        @endforeach
                    </ul>
                    <p class="text-xs text-gray-400 mt-1">Untuk mengganti lampiran, hubungi admin (fitur upload ulang
                        belum tersedia di form ini).</p>
                </div>
                @endif
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="3"
                        class="form-textarea">{{ old('catatan', $expenseReport->catatan) }}</textarea>
                </div>
            </div>

            <div class="card-body border-t flex justify-end gap-3">
                <a href="{{ route('superadmin.expense-reports.show', $expenseReport) }}"
                    class="btn-secondary btn">Batal</a>
