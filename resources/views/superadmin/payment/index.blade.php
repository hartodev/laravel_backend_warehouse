...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <a href="{{ route('payments.create') }}" class="btn-primary btn">
...
<div class="card mb-5">
    <form method="GET" class="card-body flex flex-wrap gap-3 items-end">
        ...
        <a href="{{ route('payments.index') }}" class="btn-secondary btn">Reset</a>
    </form>
</div>

<div class="table-wrap">
    <table class="data-table">
        <thead><tr><th>No. Bayar</th><th>Tipe</th><th>Metode</th><th>Dari/Ke</th><th>Tgl.</th><th class="text-right">Nominal</th><th>Status</th><th class="text-right">Aksi</th></tr></thead>
        <tbody class="divide-y divide-gray-100">
            @forelse($payments as $p)
            <tr>
                <td><span class="font-mono text-xs font-medium text-primary-700">{{ $p->payment_number }}</span></td>
                <td><span class="badge {{ $p->payment_type === 'masuk' ? 'badge-success' : 'badge-danger' }}">{{ ucfirst($p->payment_type) }}</span></td>
                ...
                <td class="text-right font-semibold {{ $p->payment_type === 'masuk' ? 'text-green-700' : 'text-red-600' }}">
                    {{ $p->payment_type === 'masuk' ? '+' : '-' }} Rp {{ number_format($p->nominal) }}
                </td>
                <td><x-status-badge :status="$p->status" /></td>
                <td class="text-right">
                    <div class="flex items-center justify-end gap-1">
                        <a href="{{ route('payments.show', $p) }}" class="btn btn-secondary btn-sm">Detail</a>
                        @if($p->status === 'pending')
                        <form method="POST" action="{{ route('payments.verify', $p) }}" class="inline">@csrf<button class="btn btn-success btn-sm">Verifikasi</button></form>
                        @endif
                    </div>
                </td>
            </tr>
            @empty
            <tr><td colspan="8" class="text-center py-12 text-gray-400">Belum ada data pembayaran</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
