        ...
        <div class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-48">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama, SKU, barcode..." class="form-input pl-9">
                </div>
            </div>
            <div class="w-44">
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex items-center gap-3">
                            @if($p->image)
                            <img src="{{ Storage::url($p->image) }}" class="w-9 h-9 rounded-lg object-cover border border-gray-100 flex-shrink-0">
                            @else
                            <div class="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0">
                                <svg class="w-5 h-5 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10"/></svg>
                            </div>
                            @endif
                            <div class="min-w-0">
                                <a href="{{ route('products.show', $p) }}" class="font-medium text-indigo-600 hover:underline truncate block">{{ $p->name }}</a>
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="font-mono text-xs text-gray-400">{{ $p->sku }}</span>
                                    @if($p->barcode)<span class="text-xs text-gray-300">·</span><span class="font-mono text-xs text-gray-400">{{ $p->barcode }}</span>@endif
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="text-right font-medium text-sm text-green-700">Rp {{ number_format($p->selling_price) }}</td>
                    ...
                    <td class="text-right">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('products.show', $p) }}"
                               class="btn btn-secondary btn-xs" title="Detail">
                            ...
                            <form method="POST" action="{{ route('products.toggle-active', $p) }}" class="inline">
                                @csrf @method('PATCH')
                                <button type="submit" class="btn btn-xs {{ $p->is_active ? 'btn-warning' : 'btn-success' }}" title="{{ $p->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                        ...
                        <div id="del-p-{{ $p->id }}" class="hidden modal-backdrop">
                            <div class="modal-box">
                                <div class="p-6">
                                    ...
                                    <h3 class="text-center font-bold text-gray-900 dark:text-white mb-1">Hapus Produk?</h3>
                                    <p class="text-center text-sm text-gray-500 mb-6">Produk <strong>{{ $p->name }}</strong> akan dihapus permanen dan tidak dapat dikembalikan.</p>
                                    <div class="flex gap-3">
                                        <button type="button" onclick="document.getElementById('del-p-{{ $p->id }}').classList.add('hidden')"
                    ...
                    <td colspan="9" class="text-center py-16">
                        <div class="flex flex-col items-center gap-3">
                            <div class="w-16 h-16 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
                                <svg class="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10"/></svg>
                            </div>
