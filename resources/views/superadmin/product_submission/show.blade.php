...
<div class="flex items-center justify-between mb-6">
    ...
    <span
        class="badge {{ $productSubmission->status === 'approved' ? 'badge-success' : ($productSubmission->status === 'pending' ? 'badge-warning' : 'badge-danger') }}">
...
<div class="card mb-5">
    ...
    <div class="card-body grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div>
            ...
            <p
                class="font-medium {{ $productSubmission->product->is_active ?? false ? 'text-green-600' : 'text-gray-500' }}">
    ...
    <div class="card-body grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div>
            ...
            <p class="font-medium text-gray-900">
                {{ $productSubmission->reviewed_at ? \Carbon\Carbon::parse($productSubmission->reviewed_at)->isoFormat('D MMM Y, HH:mm') : '—' }}
            </p>
        </div>
    ...
    <div class="flex gap-3">
        <form method="POST" action="{{ route('product-submissions.approve', $productSubmission) }}" class="inline">
            ...
            <button type="submit" class="btn-primary btn"
                onclick="return confirm('Setujui pengajuan produk ini? Produk akan otomatis diaktifkan.')">Setujui</button>
        </form>
