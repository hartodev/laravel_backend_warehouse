...
<form method="POST" action="{{ route('purchase-orders.store') }}"
      x-data="poForm()" @submit="prepareSubmit">
    @csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="card">
                <div class="card-header"><h2 class="font-semibold text-gray-900">Informasi PO</h2></div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="number" :name="'items['+idx+'][quantity]'" x-model.number="row.qty" @input="calcRow(row)" min="1" required class="form-input text-right text-sm py-1.5 w-20">
                                </td>
                                <td>
                                    <input type="number" :name="'items['+idx+'][price]'" x-model.number="row.price" @input="calcRow(row)" min="0" required class="form-input text-right text-sm py-1.5 w-32">
                                </td>
                                <td class="text-right font-medium text-sm pr-4" x-text="'Rp '+row.total.toLocaleString('id-ID')"></td>
                                <td>
        ...
        <div class="lg:col-span-1">
            <div class="card sticky top-20">
                <div class="card-header"><h3 class="font-semibold text-gray-900">Ringkasan</h3></div>
                ...
                <div class="card-body border-t pt-3">
                    ...
                    <a href="{{ route('purchase-orders.index') }}" class="btn-secondary btn w-full justify-center mt-2">Batal</a>
                </div>
            </div>
        </div>
    </div>
...
function poForm() {
    return {
        ...
        addRow() { this.rows.push({ product_id: '', qty: 1, price: 0, total: 0 }); },
        removeRow(idx) { if (this.rows.length > 1) this.rows.splice(idx, 1); },
        calcRow(row) { row.total = row.qty * row.price; },
        setPrice(row) {
            row.price = productPrices[row.product_id] || 0;
            row.total = row.qty * row.price;
        },
        prepareSubmit() { /* normal submit */ }
    };
}
</script>
@endpush
