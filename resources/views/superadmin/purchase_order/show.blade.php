...
<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
    ...
    <div class="flex flex-wrap gap-2 print:hidden">
        ...
        <a href="{{ route('superadmin.purchase-orders.edit', $purchaseOrder) }}" class="btn-secondary btn">Edit</a>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
    ...
    <div class="card p-5">
        ...
        @if($purchaseOrder->supplier?->phone)<p class="text-sm text-gray-500">{{ $purchaseOrder->supplier->phone }}</p>
        @endif
        @if($purchaseOrder->supplier?->address)<p class="text-sm text-gray-400 mt-1">
            {{ $purchaseOrder->supplier->address }}</p>@endif
    </div>
    {{-- Info gudang --}}
    <div class="card p-5">
    ...
    <div class="card p-5 space-y-2 text-sm">
        ...
        <div class="flex justify-between"><span class="text-gray-500">Tgl.
                Order</span><span>{{ \Carbon\Carbon::parse($purchaseOrder->order_date)->isoFormat('D MMM Y') }}</span>
        </div>
        <div class="flex justify-between"><span class="text-gray-500">Tgl.
                Diharapkan</span><span>{{ $purchaseOrder->expected_date ? \Carbon\Carbon::parse($purchaseOrder->expected_date)->isoFormat('D MMM Y') : '—' }}</span>
        </div>
        <div class="flex justify-between"><span
                class="text-gray-500">Pembayaran</span><span>{{ $purchaseOrder->payment_method ? ucfirst($purchaseOrder->payment_method) : '—' }}</span>
        </div>
        @if($purchaseOrder->approved_by)
        <div class="flex justify-between"><span
                class="text-gray-500">Disetujui</span><span>{{ $purchaseOrder->approvedBy->name }}</span></div>
        @endif
    </div>
</div>
        ...
        <table class="data-table">
            ...
            <tfoot class="bg-gray-50">
                ...
                <tr>
                    ...
                    <td class="text-right px-4 py-3 text-red-600">- Rp
                        {{ number_format($purchaseOrder->discount_amount) }}</td>
                </tr>
                @endif
                <tr class="border-t-2 border-gray-200">
                    <td colspan="5" class="text-right px-4 py-3 font-bold">TOTAL</td>
                    <td class="text-right px-4 py-3 font-bold text-primary-700 text-base">Rp
                        {{ number_format($purchaseOrder->total_amount) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
        ...
        <form method="POST" action="{{ route('superadmin.purchase-orders.reject', $purchaseOrder) }}">
            ...
            <div class="px-6 py-4 border-t flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('reject-po').classList.add('hidden')"
