...
@section('breadcrumb')
<a href="{{ route('superadmin.requests.index') }}" class="text-gray-500 hover:text-gray-700">Approval Final</a>
...
<form method="POST" action="{{ route('requests.approveFinal', $request) }}">
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <input type="hidden" name="items[{{ $loop->index }}][request_item_id]"
                                value="{{ $item->id }}">
                            <input type="number" min="0" name="items[{{ $loop->index }}][approved_quantity]"
    ...
    <div class="flex items-center gap-2">
        ...
        <button type="button" onclick="document.getElementById('rej-req-{{ $request->id }}').classList.remove('hidden')"
            class="btn btn-danger">Tolak</button>
    </div>
</form>

<x-confirm-modal :id="'rej-req-' . $request->id" title="Tolak Request?"
    :message="'Request dari ' . ($request->user->name ?? '—') . ' akan ditolak.'"
    :action="route('requests.reject', $request)" method="POST" confirm-text="Tolak" confirm-class="btn-danger">
    <div class="mt-3">
