...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500">{{ $sos->total() }} SO</p>
    </div>
    <a href="{{ route('superadmin.sales-orders.create') }}" class="btn-primary btn">+ Buat SO</a>
</div>
...
<div class="card mb-5">
    <form method="GET" class="card-body flex flex-wrap gap-3 items-end">
        <div class="flex-1 min-w-48"><label class="form-label">Cari</label><input type="text" name="search"
                value="{{ request('search') }}" placeholder="Nama customer, no. SO..." class="form-input"></div>
        ...
        <div class="w-48"><label class="form-label">Gudang</label>
            <select name="warehouse_id" class="form-select">
                ...
                @foreach($warehouses as $w)<option value="{{ $w->id }}"
                    {{ request('warehouse_id') == $w->id ? 'selected' : '' }}>{{ $w->name }}</option>@endforeach
            </select>
        </div>
        ...
        <a href="{{ route('superadmin.sales-orders.index') }}" class="btn-secondary btn">Reset</a>
    </form>
</div>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="text-xs text-gray-400 truncate max-w-40">{{ $so->customer_address }}</p>
                </td>
                ...
                <td class="text-right">
                    <div class="flex items-center justify-end gap-1">
                        <a href="{{ route('superadmin.sales-orders.show', $so) }}"
                            class="btn btn-secondary btn-sm">Detail</a>
                        @if($so->status === 'draft')
                        <a href="{{ route('superadmin.sales-orders.edit', $so) }}"
            ...
            @empty
            <tr>
                <td colspan="8" class="text-center py-12 text-gray-400">Belum ada Sales Order</td>
            </tr>
            @endforelse
        </tbody>
    </table>
