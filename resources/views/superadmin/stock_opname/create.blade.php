        ...
        <form method="POST" action="{{ route('superadmin.stock-opnames.store') }}">
            ...
            <div class="card-body space-y-4">
                ...
                <div>
                    ...
                    <input type="date" name="opname_date" value="{{ old('opname_date', date('Y-m-d')) }}" required class="form-input @error('opname_date') border-red-400 @enderror">
                    @error('opname_date')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="form-label">Catatan</label>
                    <textarea name="notes" rows="3" class="form-textarea" placeholder="Opsional...">{{ old('notes') }}</textarea>
                </div>
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
                    💡 Setelah membuat opname, klik <strong>Mulai Opname</strong> untuk mengisi stok fisik. Semua produk di gudang akan otomatis dimuat.
                </div>
            </div>
