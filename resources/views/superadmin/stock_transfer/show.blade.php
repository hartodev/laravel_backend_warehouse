...
<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Diajukan oleh:
            <strong>{{ $stockTransfer->requestedBy->name ?? '—' }}</strong> ·
            {{ $stockTransfer->created_at->isoFormat('D MMM Y, HH:mm') }}</p>
    </div>
    <div class="flex flex-wrap gap-2">
        ...
        <button onclick="document.getElementById('resolve-modal').classList.remove('hidden')"
            class="btn-primary btn">Resolusi Selisih</button>
        @endif
    </div>
</div>
...
<div class="card p-4 mb-5 bg-red-50 border-red-100">
    ...
    <p class="text-xs text-red-400 mt-1">Oleh {{ $stockTransfer->cancelledBy->name ?? '—' }} ·
        {{ $stockTransfer->cancelled_at?->isoFormat('D MMM Y, HH:mm') }}</p>
</div>
@endif
...
<div class="card p-4 mb-5 bg-yellow-50 border-yellow-100">
    ...
    <p class="text-xs text-yellow-500 mt-1">Oleh {{ $stockTransfer->discrepancyReportedBy->name ?? '—' }} ·
        {{ $stockTransfer->discrepancy_reported_at?->isoFormat('D MMM Y, HH:mm') }}</p>
</div>
@endif
...
<div class="card p-4 mb-5 bg-blue-50 border-blue-100">
    ...
    <p class="text-xs text-blue-400 mt-1">Oleh {{ $stockTransfer->resolvedBy->name ?? '—' }} ·
        {{ $stockTransfer->resolved_at?->isoFormat('D MMM Y, HH:mm') }}</p>
</div>
@endif
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5 text-sm">
    <div class="card p-4">
        ...
        <p class="font-medium">{{ \Carbon\Carbon::parse($stockTransfer->transfer_date)->isoFormat('D MMM Y') }}</p>
    </div>
    <div class="card p-4">
        <p class="text-gray-400 text-xs mb-1">Est. Tiba</p>
        <p class="font-medium">
            {{ $stockTransfer->expected_arrival ? \Carbon\Carbon::parse($stockTransfer->expected_arrival)->isoFormat('D MMM Y') : '—' }}
        </p>
    </div>
    <div class="card p-4">
        <p class="text-gray-400 text-xs mb-1">Dikirim</p>
        <p class="font-medium">
            {{ $stockTransfer->sent_at ? \Carbon\Carbon::parse($stockTransfer->sent_at)->isoFormat('D MMM Y') : '—' }}
        </p>
    </div>
    <div class="card p-4">
        <p class="text-gray-400 text-xs mb-1">Diterima</p>
        <p class="font-medium">
            {{ $stockTransfer->received_at ? \Carbon\Carbon::parse($stockTransfer->received_at)->isoFormat('D MMM Y') : '—' }}
        </p>
    </div>
</div>
...
<div class="card">
    <div class="card-header">
        <h3 class="font-semibold text-gray-900">Item Transfer</h3>
    </div>
        ...
        <table class="data-table">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr>
                    ...
                    <td
                        class="text-right {{ $item->quantity_sent < $item->quantity_requested ? 'text-yellow-600' : '' }}">
                        {{ number_format($item->quantity_sent) }}</td>
                    <td
                        class="text-right {{ $item->quantity_received < $item->quantity_sent ? 'text-yellow-600' : 'text-green-700' }}">
                        {{ number_format($item->quantity_received) }}</td>
                ...
                @empty
                <tr>
                    <td colspan="7" class="text-center py-8 text-gray-400">Tidak ada item</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <form method="POST" action="{{ route('stock-transfers.resolve-discrepancy', $stockTransfer) }}">
            ...
            <div class="px-6 py-4 space-y-3">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" required class="form-textarea"
                        placeholder="Jelaskan keputusan..."></textarea>
                </div>
            </div>
            <div class="px-6 py-4 border-t flex justify-end gap-3">
