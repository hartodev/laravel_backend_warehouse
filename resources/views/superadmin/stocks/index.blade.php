...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('superadmin.stocks.low-stock') }}" class="btn btn-secondary">⚠ Stok Menipis</a>
    </div>
</div>
...
<div class="card mb-5">
    <form method="GET" class="card-body flex flex-wrap gap-3 items-end">
        <div class="w-64">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama atau SKU produk..."
                class="form-input">
        </div>
        <div class="w-48">
        ...
        <a href="{{ route('superadmin.stocks.index') }}" class="btn btn-secondary">Reset</a>
    </form>
</div>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            @php $isLow = $stock->quantity <= ($stock->product->min_stock ?? 0); @endphp
                <tr class="{{ $isLow ? 'bg-red-50' : '' }}">
                    ...
                    <td class="text-right font-semibold">{{ number_format($stock->quantity) }}
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-12 text-gray-400">Belum ada data stok</td>
                </tr>
                @endforelse
        </tbody>
    </table>
