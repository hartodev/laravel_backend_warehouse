...
<div class="space-y-5">
    ...
    {{-- Info Utama --}}
    <div class="card">
        <div class="card-header"><h2 class="font-semibold text-gray-900 dark:text-white">Informasi Supplier</h2></div>
        <div class="card-body grid grid-cols-2 gap-4">
            <div class="col-span-2">
                <label class="form-label">Nama Supplier <span class="text-red-500">*</span></label>
                <input type="text" name="name" value="{{ old('name') }}" required class="form-input @error('name') border-red-400 @enderror" placeholder="Nama perusahaan / individu">
                @error('name')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="form-label">Email</label>
                <input type="email" name="email" value="{{ old('email') }}" class="form-input @error('email') border-red-400 @enderror" placeholder="supplier@email.com">
                @error('email')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="form-label">No. Telepon</label>
                <input type="text" name="phone" value="{{ old('phone') }}" class="form-input" placeholder="08xx">
            </div>
            <div>
                <label class="form-label">Kota</label>
                <input type="text" name="city" value="{{ old('city') }}" class="form-input" placeholder="cth. Jakarta">
            </div>
            <div class="col-span-2">
    ...
    {{-- Kontak PIC --}}
    <div class="card">
        <div class="card-header"><h2 class="font-semibold text-gray-900 dark:text-white">Contact Person</h2></div>
        <div class="card-body grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
                ...
                <input type="text" name="contact_phone" value="{{ old('contact_phone') }}" class="form-input" placeholder="08xx">
            </div>
        </div>
    </div>
    ...
    {{-- Info Bank --}}
    <div class="card">
        <div class="card-header"><h2 class="font-semibold text-gray-900 dark:text-white">Informasi Bank</h2></div>
        <div class="card-body grid grid-cols-3 gap-4">
            <div>
                <label class="form-label">Nama Bank</label>
                <input type="text" name="bank_name" value="{{ old('bank_name') }}" class="form-input" placeholder="BCA, BNI, dll">
            </div>
            <div>
                <label class="form-label">No. Rekening</label>
                <input type="text" name="bank_account" value="{{ old('bank_account') }}" class="form-input font-mono">
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="card">
            <div class="card-header"><h2 class="font-semibold text-gray-900 dark:text-white">Logo</h2></div>
            <div class="card-body" x-data="{ preview: null }">
                <div class="border-2 border-dashed border-gray-200 dark:border-slate-600 rounded-xl p-4 text-center cursor-pointer hover:border-indigo-400 transition-colors"
                    ...
                    <img x-show="preview" :src="preview" class="w-24 h-24 object-cover rounded-lg mx-auto mb-2">
                    <div x-show="!preview" class="py-4">
                        <svg class="w-8 h-8 text-gray-300 mx-auto mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        ...
        <div class="card">
            <div class="card-header"><h2 class="font-semibold text-gray-900 dark:text-white">Catatan & Status</h2></div>
            <div class="card-body space-y-4">
                <div>
                    <label class="form-label">Catatan</label>
                    <textarea name="notes" rows="4" class="form-input resize-none" placeholder="Catatan tambahan tentang supplier...">{{ old('notes') }}</textarea>
                </div>
