...
@section('breadcrumb')
<a href="{{ route('superadmin.suppliers.index') }}" class="text-gray-500 hover:text-gray-700">Supplier</a>
...
<form method="POST" action="{{ route('suppliers.update', $supplier) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2"
                        class="form-textarea @error('address') is-invalid @enderror">{{ old('address', $supplier->address) }}</textarea>
                    @error('address')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="contact_person"
                        ...
                        class="form-input @error('contact_person') is-invalid @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $supplier->code) }}"
                        class="form-input @error('code') is-invalid @enderror" placeholder="Contoh: SUP-001" required>
                    @error('code')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="bank_account" value="{{ old('bank_account', $supplier->bank_account) }}"
                        class="form-input @error('bank_account') is-invalid @enderror">
                    @error('bank_account')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="text" name="bank_holder" value="{{ old('bank_holder', $supplier->bank_holder) }}"
                        class="form-input @error('bank_holder') is-invalid @enderror">
                    @error('bank_holder')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="card-body">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">JPG, PNG, atau WEBP. Maks. 2MB. Kosongkan jika tidak ingin
                        mengubah logo.</p>
                    @error('logo')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                        class="form-textarea @error('notes') is-invalid @enderror">{{ old('notes', $supplier->notes) }}</textarea>
                    @error('notes')
                    <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-4 flex items-center gap-2">
                ...
                <input type="checkbox" id="is_active" name="is_active" value="1"
                    {{ old('is_active', $supplier->is_active) ? 'checked' : '' }} class="form-checkbox">
    ...
    <div class="flex items-center justify-end gap-2">
        <a href="{{ route('superadmin.suppliers.index') }}" class="btn btn-secondary">Batal</a>
