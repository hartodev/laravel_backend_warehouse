...
<div class="card">
    <div class="table-wrap border-0 rounded-xl">
        <table class="data-table">
            <thead>
                <tr>
                    <th>#</th><th>Nama Supplier</th><th>Kontak</th><th>Kota</th>
                    <th>Bank</th><th class="text-right">Produk</th><th>Status</th>
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex items-center gap-3">
                            @if($sup->logo)
                            <img src="{{ Storage::url($sup->logo) }}" class="w-8 h-8 rounded-lg object-cover border flex-shrink-0">
                            @else
                            <div class="w-8 h-8 rounded-lg bg-purple-50 flex items-center justify-center flex-shrink-0 text-purple-300 font-bold text-sm">
                                {{ strtoupper(substr($sup->name,0,1)) }}
                            </div>
                            @endif
                            <div>
                                <a href="{{ route('suppliers.show', $sup) }}" class="font-medium text-indigo-600 hover:underline">{{ $sup->name }}</a>
                    ...
                    <td><span class="badge {{ $sup->is_active ? 'badge-success' : 'badge-gray' }}">{{ $sup->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                    <td class="text-right">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('suppliers.show', $sup) }}" class="btn btn-secondary btn-xs">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            <a href="{{ route('suppliers.edit', $sup) }}" class="btn btn-secondary btn-xs">
                            ...
                            <button onclick="document.getElementById('del-sup-{{ $sup->id }}').classList.remove('hidden')" class="btn btn-danger btn-xs">
                        ...
                        <div id="del-sup-{{ $sup->id }}" class="hidden modal-backdrop">
                            <div class="modal-box p-6">
                                ...
                                <h3 class="text-center font-bold text-gray-900 dark:text-white mb-1">Hapus Supplier?</h3>
                                <p class="text-center text-sm text-gray-500 mb-5"><strong>{{ $sup->name }}</strong> akan dihapus permanen.</p>
                                <div class="flex gap-3">
                                    <button onclick="document.getElementById('del-sup-{{ $sup->id }}').classList.add('hidden')" class="btn btn-secondary flex-1 justify-center">Batal</button>
    ...
    <div class="card-body border-t border-gray-100 dark:border-slate-700">{{ $suppliers->withQueryString()->links() }}</div>
    @endif
</div>
@endsection
