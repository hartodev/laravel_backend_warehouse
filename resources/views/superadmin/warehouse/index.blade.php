...
<div class="card mb-5">
    <form method="GET" class="card-body flex flex-wrap gap-3 items-end">
        <div class="w-64">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama, kode, atau lokasi..." class="form-input">
        </div>
        <div class="w-40">
        ...
        <a href="{{ route('warehouses.index') }}" class="btn btn-secondary">Reset</a>
    </form>
</div>
...
<div class="table-wrap">
    <table class="data-table">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr>
                ...
                <td class="text-right">
                    <div class="flex items-center justify-end gap-1">
                        <a href="{{ route('warehouses.show', $warehouse) }}" class="btn btn-secondary btn-sm">Detail</a>
                        <a href="{{ route('warehouses.edit', $warehouse) }}" class="btn btn-primary btn-sm">Edit</a>
                        <button onclick="window.dispatchEvent(new CustomEvent('open-modal-del-warehouse-{{ $warehouse->id }}'))" class="btn btn-danger btn-sm">Hapus</button>
                    </div>

                    <x-confirm-modal
                        id="del-warehouse-{{ $warehouse->id }}"
                        title="Hapus Gudang?"
            ...
            @empty
            <tr><td colspan="6" class="text-center py-12 text-gray-400">Belum ada data gudang</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
